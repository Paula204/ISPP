...
($ => {
    ...
    class MatchResult {
        ...
        private static emptyTeam(source: () => TeamBlock, sibling: TeamBlock): TeamBlock {
            const teamBlock = new TeamBlock(source, Option.empty(), Option.empty(), Option.empty(), Score.empty());
    ...
    const winnerMatchSources = (teams: Array<[Option<object>, Option<object>]>, m: number) => (): [MatchSource, MatchSource] => {
        const teamA = new TeamBlock(endOfBranch, () => teams[m][0], Option.of(Order.first()), Option.of<number>(m * 2), Score.empty());
        const teamB = new TeamBlock(endOfBranch, () => teams[m][1], Option.of(Order.second()), Option.of<number>(m * 2 + 1), Score.empty());
    ...
    function prepareWinners(
        ...
        if (isSingleElimination) {
            ...
            if (teams.length > 1 && !opts.skipConsolationRound) {
                ...
                const consol = round.addMatch(() => [{ source: third }, { source: fourth }], Option.of(consolationBubbles));
    ...
    function prepareFinals(finals: Bracket, winners: Bracket, losers: Bracket, opts: Options, topCon: JQuery, resizeContainer: () => void) {
        const round = finals.addRound(Option.empty());
        const finalMatch = round.addMatch(
            () => [{ source: () => winners.winner() }, { source: () => losers.winner() }],
            Option.of(match => {
                ...
                if (!opts.skipSecondaryFinal && (!match.winner().name.isEmpty() && match.winner().name === losers.winner().name)) {
                    ...
                    const finalRound = finals.addRound(Option.of<BoolCallback>(doRenderCb));
                    /* keep order the same, WB winner top, LB winner below */
                    const match2 = finalRound.addMatch(
                        () => [{ source: () => match.first() }, { source: () => match.second() }],
                        Option.of(winnerBubbles)
                    );

                    match.setConnectorCb(Option.of(tC => ({ height: 0, shift: tC.height() / 2 })));
        ...
        return team.name
            .map(() =>
                score
                    ...
                    .orElseGet(() => (opponent.emptyBranch() === BranchType.BYE ? EntryState.ENTRY_DEFAULT_WIN : EntryState.ENTRY_NO_SCORE))
            )
            .orElseGet(() => {
    ...
    class Round {
        ...
        public addMatch(teamCb: (() => [MatchSource, MatchSource]) | null, renderCb: Option<RenderCallback>): Match {
            ...
            const teamABlock = new TeamBlock(teamA, teamA().name, Option.of(Order.first()), teamA().seed, Score.empty());
            const teamBBlock = new TeamBlock(teamB, teamB().name, Option.of(Order.second()), teamB().seed, Score.empty());
        ...
        public results(): ResultObject[] {
            return this.matches.reduce((agg: ResultObject[], m) => agg.concat([m.results()]), []);
        }
    }
    ...
    class Bracket {
        ...
        public addRound(doRenderCb: Option<BoolCallback>): Round {
            ...
            const roundResults = this.initResults.map<ResultObject[]>(r =>
                r[id] === undefined ? new ResultObject(Score.empty(), Score.empty(), undefined) : r[id]
            );
       ...
       Therefore length must be read on each iteration. */
        ...
        public results(): ResultObject[][] {
            return this.rounds.reduce((agg: ResultObject[][], r) => agg.concat([r.results()]), []);
        }
    }
    ...
    function mkConnector(roundMargin: number, connector: Connector, teamCon: JQuery, align: string): JQuery {
    ...
    function teamElement(
        ...
        const score = team.name.isEmpty() || opponent.name.isEmpty() || !isReady ? Option.empty() : team.score.map(s => `${s}`);
            ...
            brackets.map(rounds =>
                rounds.map(
                    (matches: [number, number, any]) =>
                        new ResultObject(Score.of(undefinedToNull(matches[0])), Score.of(undefinedToNull(matches[1])), matches[2])
                )
            )
        );
    ...
    const methods = {
        init(originalOpts: Options) {
            ...
            if (log2Result !== Math.floor(log2Result)) {
                $.error(
                    `"teams" property must have 2^n number of team pairs, i.e. 1, 2, 4, etc. Got ${opts.init.teams.length} team pairs.`
                );
            }
